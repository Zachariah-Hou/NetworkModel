...
2 计算得到checksum之后   赋值时要转换为网络字节序: ip_header.checksum = htons(checksum);
...
伪头部：4 byte源ip地址 + 4 byte目的ip地址 + 0x00 + 1 byte协议 + udp长度（udp包头长度+数据长度）
udp包头：2 byte源端口 + 2 byte目的端口 + 2 byte udp包长（包头自带的长度） + 0x0000(checksum)
数据部分
